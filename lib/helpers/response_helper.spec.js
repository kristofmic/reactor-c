...
describe('request helper', function() {
  ...
  describe('handleError', function() {
    it('should send the error', function() {
      ...
      responseHelper.handleError(resStub)(testError);

      expect(resStub.status).to.have.been.calledWith(401);
      expect(resStub.json).to.have.been.calledWith({ message: testError.message });
    });
  });
});